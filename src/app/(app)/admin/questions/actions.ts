...
export async function insertQuestion(
  input: QuestionInput,
  sourceBatch: string,
  status: "active" | "qa_pending" = "active",
  extra: { citationsJson?: string | null; support?: "high" | "medium" | "low" | null } = {},
) {
  ...
  const [q] = await db
    ...
    .values({
      examId,
      syllabusItemId: input.syllabus_item_id,
      citationsJson: extra.citationsJson ?? null,
...
export async function saveManualQuestion(_prev: SaveResult | null, formData: FormData): Promise<SaveResult> {
  ...
  const raw = {
    syllabus_item_id: Number(formData.get("syllabus_item_id")),
